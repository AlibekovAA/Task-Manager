...
def admin_headers(client):
    ...
    response = client.post(
        "/users/",
        json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD, "secret_word": ADMIN_SECRET_WORD, "is_admin": True},
    )
    assert response.status_code == 200, f"Admin creation failed: {response.json()}"

    login_response = client.post("/token", data={"username": ADMIN_EMAIL, "password": ADMIN_PASSWORD})
...
def test_update_nonexistent_user_role(client, admin_headers):
    response = client.put("/users/999999/role", headers=admin_headers, json={"role": "admin"})
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_update_admin_role(client, admin_headers):
    admin_response = client.post(
        "/users/",
        json={"email": "admin2@test.com", "password": "adminpass123", "secret_word": "secret", "is_admin": True},
    )
    admin_id = admin_response.json()["id"]

    response = client.put(f"/users/{admin_id}/role", headers=admin_headers, json={"role": "user"})
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_unauthorized_role_update(client):
    ...
    response = client.put(f"/admin/users/{user_id}/role", headers={}, json={"role": "admin"})
...
def test_pm_role_task_management(client, admin_headers):
    ...
    client.put(f"/users/{pm_id}/role", headers=admin_headers, json={"role": "pm"})

    pm_token = client.post("/token", data={"username": "pm@example.com", "password": "pmpass123"}).json()[
        "access_token"
    ]
...
def test_role_hierarchy(client, admin_headers):
    ...
    for role in roles:
        ...
        response = client.post("/users/", json={"email": email, "password": "testpass123", "secret_word": "secret"})
        user_id = response.json()["id"]

        if role != "user":
            client.put(f"/users/{user_id}/role", headers=admin_headers, json={"role": role})

        token_response = client.post("/token", data={"username": email, "password": "testpass123"})
