...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    return _handle_password_operation(
        "verify", plain_password, hashed_password, log_msg="Password verification completed"
    )


def get_password_hash(password: str) -> str:
    result = _handle_password_operation("hash", password, log_msg="Password hashed successfully")
    if result is None:
...
def refresh_access_token(refresh_token: str):
    try:
        ...
        return create_access_token(
            data={"sub": email}, expires_delta=timedelta(minutes=int(ACCESS_TOKEN_EXPIRE_MINUTES))
        )
