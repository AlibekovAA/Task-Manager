document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function updateAdminMenuItem() {
    const userData = await loadUserProfile();
    if (userData && adminMenuItem) {
  ...
  async function loadTasks() {
    try {
      ...
      if (userData.role === 'pm') {
        const assignedResponse = await fetchWithToken('/assigned-tasks/');
      ...
      showNotification(
        'Ошибка при загрузке задач. Попробуйте обновить страницу',
        'error',
      );
    }
  }
  ...
  function renderTasks(tasks) {
    ...
    if (!tasks || tasks.length === 0) {
      tasksList.innerHTML =
        '<div class="no-tasks-message">Список задач пуст. Нажмите "Добавить задачу" чтобы создать новую задачу.</div>';
      return;
    }
    ...
    tasks.forEach((task) => {
      ...
      const columnKey =
        status === 0 ? 'backlog' : status === 1 ? 'inProgress' : 'completed';
      columns[columnKey]
  ...
  async function handleDrop(e) {
    ...
    try {
      const response = await fetchWithToken(`/tasks/${taskId}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        await loadTasks();
        showNotification(
          `Задача успешно ${getStatusLabel(newStatus).toLowerCase()}`,
          'success',
        );
      } else {
  ...
  function createTaskCard(task) {
    ...
    if (task.due_date && task.status !== 2) {
      try {
        ...
        if (task.status === 1) {
          const progress = calculateFuseProgress(
            task.due_date,
            task.created_at,
          );
    ...
    taskElement.innerHTML = `
            <div class="task-info">
                <div class="task-header">
                    ...
                    <span class="priority-${
                      task.priority || 3
                    }">${getPriorityLabel(task.priority || 3)}</span>
                </div>
                <p>${task.description || ''}</p>
                <p class="task-status">Статус: ${getStatusLabel(
                  task.status,
                )}</p>
                ${
                  dueDate
            ...
            <div class="task-actions">
                <button class="action-btn files-btn" onclick="openFilesModal(${
                  task.id
                })" title="Фалы задачи">
                    <i class="fas fa-paperclip"></i>
                    ${
                      task.files?.length
                        ? `<span class="files-count">${task.files.length}</span>`
                        : ''
                    }
                </button>
                ${
                ...
                <button class="action-btn delete-btn" onclick="showDeleteConfirmModal(${
                  task.id
                })">
      ...
      if (response.ok) {
        ...
        if (userRole === 'pm' && taskAssignee) {
          ...
          if (assigneeId) {
            ...
            const currentUserOption = Array.from(taskAssignee.options).find(
              (option) => option.textContent.includes(currentUserEmail),
            );
        ...
        document.querySelector('#taskModal h3').textContent =
  ...
  async function createTask(taskData) {
    try {
      const userId = userRole === 'pm' ? parseInt(taskAssignee.value) : null;
      const response = await fetchWithToken('/tasks/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const errorMsg = getErrorMessage(error, 'Ошибка при создании задачи');
  ...
  taskForm.addEventListener('submit', async (e) => {
    ...
    if (userRole === 'pm' && !taskAssignee.value) {
      showNotification(
        'Пожалуйста, выберите пользователя для назначения задачи',
        'error',
      );
      return;
    }
  ...
  async function loadDefaultUsers(currentUserId) {
    try {
      ...
      if (response.ok) {
        ...
        if (currentPM) {
          const optionsHtml = defaultUsers
    ...
    type.addEventListener('click', () => {
      ...
      filterOptions.forEach((option) => {
        option.style.display =
          option.id === `${filterType}Filter` ? 'block' : 'none';
      });
    });
  });
  ...
  document
    .querySelector('#priorityFilter select')
  ...
  document
    .querySelector('#statusFilter select')
    .addEventListener('change', (e) => {
  ...
  function applyFilters() {
    ...
    tasks.forEach((task) => {
      ...
      if (currentFilter.type === 'priority' && currentFilter.value) {
        const priority = task
          .querySelector('[class^="priority-"]')
          .className.match(/priority-(\d)/)[1];
      ...
      if (currentFilter.type === 'status' && currentFilter.value) {
        ...
        show =
          (currentFilter.value === 'completed' && isCompleted) ||
  ...
  async function loadTaskFiles(taskId) {
    try {
      const response = await fetchWithToken(`/tasks/${taskId}/files`);
    ...
    files.forEach((file) => {
      ...
      fileItem.innerHTML = `
                ...
                <div class="file-actions">
                    <button class="download-btn" onclick="downloadFile(${currentTaskId}, ${
        file.id
      }, '${file.filename}')">
                        <i class="fas fa-download"></i>
                    </button>
                    <button class="delete-btn" onclick="deleteFile(${currentTaskId}, ${
        file.id
      })">
  ...
  fileUploadForm.onsubmit = async (e) => {
    ...
    try {
      const response = await fetchWithToken(`/tasks/${currentTaskId}/files/`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        const errorMsg = getErrorMessage(error, 'Ошибка при загрузке файла');
        showNotification(errorMsg, 'error');
      }
    } catch (error) {
  ...
  async function downloadFile(taskId, fileId, filename) {
    try {
      const response = await fetchWithToken(`/tasks/${taskId}/files/${fileId}`);
